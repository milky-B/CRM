...
public class ActivityController {
    ...
    public Object save(Activity activity, HttpServletRequest request) {
        ...
        OperateLog log = new OperateLog(user.getId(), "新增活动", user.getName(), activity.getId());
    ...
    public Object delete(String[] ids) {
        ...
        User user = (User)((ServletRequestAttributes) RequestContextHolder.currentRequestAttributes()).getRequest().getSession().getAttribute(ConstantsMessage.Session_User);
        operateLogMapper.insert(new OperateLog(user.getId(), "删除活动",user.getName(),Arrays.stream(ids).collect(Collectors.joining(","))));
        return returnMessage;
    }
    ...
    public Object update(Activity activity, HttpServletRequest request) {
        ...
        User user = (User)((ServletRequestAttributes) RequestContextHolder.currentRequestAttributes()).getRequest().getSession().getAttribute(ConstantsMessage.Session_User);
        operateLogMapper.insert(new OperateLog(user.getId(), "更新活动",user.getName(),activity.getId()));
        return returnMessage;
    }
    /*
    * 大文件上传下载
    * */
    ...
    @RequestMapping("/activity/funnel.do")
    @ResponseBody
    public List<Funnel> getActivityFunnel(){
        List<Funnel> funnels = activityService.selectFunnelByStage();
        return funnels;
    }
}